...
/**
 * @brief Clear ALL the data in the block (incl. overflow data) by marking all
 * tuples' tombstones. Set mainblock_occupied = 0, overflow_occupied = 0,
 * tombstones_number = 0.
 *
...
int block::clear() {
  ...
  for (int i = 0; i < mainblock_size; i++) mainblock[i].mark_tombstone();
  for (int i = 0; i < overflow_size; i++) overflow[i].mark_tombstone();
  return 1;
}
 ...
 * Person with marked tombstone is always greater than Person with unmarked
 * tombstone.
 *
 * @param record1 record 1
 * @param record2  record 2
 * @returns True if the primary key (ID) of record1 is less than the primary key
 * (ID) of record2, false otherwise.
 */
...
/**
 * @brief Sorts all tuples in the main and the overflow block and stores the
 * result only in the main block, leving the overflow block empty. This function
 * use sort function in algorithm library. After sort, tombstones_number = 0
 *
 * @return 1 for indication.
 */
int block::sort(void) {
  int first_empty = 0;
  int i, indicator;
  for (i = 0, indicator = 0; i < overflow_size && indicator < overflow_occupied;
       i++) {
    ...
    (overflow + i)->mark_tombstone();  // Remove tuple in the overflow block by
                                       // marking the tombstone after copy.
    indicator++;
  }
  std::sort(mainblock,
            mainblock + (mainblock_occupied +
                         std::max(overflow_occupied, tombstones_number)),
            cmp4sort);
...
/**
 * @brief Combines binary search in a main block with linear search in the
 * overflow block.
 *
 * @param ID ID
...
/**
 * @brief Inserts a new tuple into the overflow block at the first free space.
 * If the overflow block is full, trigger the sorting. If after the sorting
 * there are too many tuples in the main block, split the block.
 * @param tuple tuple
...
block *block::insert(Person *tuple) {
  ...
  overflow_occupied++;
  if (overflow_size == overflow_occupied) sort();
  // After split is called, all the tuples are in mainblock, and there is no
  // tombstone.
...
block *block::split(void) {
  ...
  for (int i = mid; i < mainblock_occupied; i++) {
    new_block->mainblock[(new_block->mainblock_occupied)++] =
        this->mainblock[i];
 ...
 *  If after remove there are too few tuples in the main block, merge the block
 * with the next block if the next block exists.
 * @param ID ID
...
block *block::remove(string ID) {
  ...
  int lowPos = 0,
      highPos =
          tombstones_number + mainblock_occupied - 1;  // all data in block
  int mid, shift;
  while (highPos >= lowPos && mainblock_occupied != 0) {
    ...
    if (mainblock[shift].get_key() == ID) {
      ...
      if ((this->mainblock_occupied < merge_threshold) &&
          (nullptr != this->nextPointer()) &&
          (0 < this->nextPointer()->mainblock_occupied)) {
        return merge();
      } else {
        return NULL;
      }
...
/**
 * @brief Merges a block with one neighbouring block and either splits the
 * result more equally or removes one block. This function will create a new
 * separation key.
 * @note Assume that merge will be called only when the number of this block is
 * less than the merge_threshold.
 * @WARNING the neighbouring block might be deleted. All the keys in both blocks
...
block *block::merge(void) {
  ...
  int total_num_tuples = this->mainblock_occupied + this->overflow_occupied +
                         neighbour->mainblock_occupied +
                         neighbour->overflow_occupied;
  ...
  int i, indicator;
  for (i = 0, indicator = 0;
       i < this->mainblock_size && indicator < this->mainblock_occupied; i++) {
  ...
  for (i = 0, indicator = 0; i < neighbour->mainblock_size &&
                             indicator < neighbour->mainblock_occupied;
       i++) {
  ...
  for (i = 0, indicator = 0;
       i < this->overflow_size && indicator < this->overflow_occupied; i++) {
  ...
  for (i = 0, indicator = 0;
       i < neighbour->overflow_size && indicator < neighbour->overflow_occupied;
       i++) {

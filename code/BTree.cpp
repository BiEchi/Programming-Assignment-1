...
CommonTreeNode *BTree::btree_node_new()
{
	...
	node->labelArrayForBTree = vector<string>(2 * M - 1, "0");
...
void BTree::btree_insert_nonfull(CommonTreeNode *node, Person *target)
{
	...
	{
		int pos = node->num;
		while (pos >= 1 && target->getName() < database->find(node->labelArrayForBTree[pos - 1])->getName())
		{
			node->labelArrayForBTree[pos] = node->labelArrayForBTree[pos - 1];
			pos--;
		}
		node->labelArrayForBTree[pos] = target->getID();
		node->num++;
		btree_node_num += 1;
	}
	else
	{
		int pos = node->num;
		while (pos > 0 && target->getName() < database->find(node->labelArrayForBTree[pos - 1])->getName())
			pos--;

		if (2 * M - 1 == node->ptrArray[pos]->num)
		{
			btree_split_child(node, pos, node->ptrArray[pos]);
			if (target->getName() > database->find(node->labelArrayForBTree[pos])->getName())
				pos++;
		}
...
void BTree::btree_delete_nonone(CommonTreeNode *root, Person *target)
{
	...
	{
		int i = 0;
		while (i < root->num && target->getName() > database->find(root->labelArrayForBTree[i])->getName())
			i++;
		if (target->getName() == database->find(root->labelArrayForBTree[i])->getName())
		{
	...
	else
	{
		...
		while (i < root->num && target->getName() > database->find(root->labelArrayForBTree[i])->getName())
			i++;
		if (i < root->num && target->getName() == database->find(root->labelArrayForBTree[i])->getName())
		{
			y = root->ptrArray[i];
			z = root->ptrArray[i + 1];
			if (y->num > M - 1)
			{
				string ID = btree_search_predecessor(y);
				Person *pre = database->find(ID);
				database->find(root->labelArrayForBTree[i])->setName(pre->getName());
				btree_delete_nonone(y, pre);
			}
			else if (z->num > M - 1)
			{
				string ID = btree_search_successor(z);
				Person *next = database->find(ID);
				database->find(root->labelArrayForBTree[i])->setName(next->getName());
...
void BTree::btree_level_display(CommonTreeNode *root)
{
	...
	while (front < rear)
	{
		...
		{
			cout << database->find(node->labelArrayForBTree[i])->getName() << " ";
		}
		printf("]");
